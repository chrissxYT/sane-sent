...
static void getfontsize(Slide *s, unsigned *width, unsigned *height);
...
{
	int fds[2];
	if (pipe(fds) < 0) die("sent: Unable to create pipe:");
	switch (fork()) {
	case -1:
...
void ffload(Slide *s)
{
	...
	for (off = 0, y = 0; y < s->img->bufheight; y++) {
		...
		while (nbytes < rowlen) {
			...
			if (count < 0) die("sent: Unable to read from pipe:");
			nbytes += count;
		}
...
void ffprepare(Image *img)
{
	...
	else height = img->bufheight * xw.uw / img->bufwidth;
...
{
	unsigned x, y;
	unsigned width = img->ximg->width;
	unsigned height = img->ximg->height;
	char* newBuf = img->ximg->data;
	unsigned char* ibuf;
	unsigned jdy = img->ximg->bytes_per_line / 4 - width;
	unsigned dx = (img->bufwidth << 10) / width;

	for (y = 0; y < height; y++) {
		unsigned bufx = img->bufwidth / width;
...
void
getfontsize(Slide *s, unsigned *width, unsigned *height)
{
	int i, j;
	unsigned curw, newmax;
