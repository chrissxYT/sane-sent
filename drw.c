...
static size_t utf8decode(const char *c,
			 long *u,
			 size_t clen)
{
	size_t i, j, len, type;
 ...
 */
static Fnt *xfont_create(Drw *drw,
			 const char *fontname,
...
void drw_font_getexts(Fnt *font,
		      const char *text,
		      unsigned len,
		      unsigned *w,
		      unsigned *h)
...
Cursor *drw_cur_create(Drw *drw, int shape)
{
	...
	if (!drw || !(cur = ecalloc(1, sizeof(Cursor))))
		return NULL;
